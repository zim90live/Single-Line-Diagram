...
- 顶部/左侧标尺、画布平移和缩放。
- 属性面板编辑名称、设备标识、坐标、尺寸和角度；2WV、CV、Generator、Grid、MP、Switch、Transformer 可修改实例颜色。母线与子线支持同类多选或混合选择后批量改色；空选时按图元、母线、子线汇总当前画布颜色，可按同类同色对象全局替换。
- 每个图元始终显示设备标识；系统自动在四侧避让锚点出线区域，单选图元后可拖动标签改变侧向，旋转不改变文字方向或侧向。
...
当前注册 `src/assets/symbols/` 中的 21 个 SVG 文件，呈现为 20 个逻辑图元：`SwitchOn.svg` 与 `SwitchOff.svg` 统一为默认 Off 的 `Switch`，On/Off 切换留给后续监控运行态。编辑模式已覆盖图元、锚点、普通接线和电力母线；监控模式仍后置。
