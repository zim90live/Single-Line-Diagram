...
interface RouteEndpointInset {
  x: number
  y: number
}

function terminalInsetTowardAdjacentPoint(
  terminal: Point,
  adjacent: Point | undefined,
  gridSize: number,
): RouteEndpointInset | null {
  if (!adjacent) return null
  const xDirection = Math.sign(adjacent.x - terminal.x)
  const yDirection = Math.sign(adjacent.y - terminal.y)
  if ((xDirection === 0) === (yDirection === 0)) return null
  return { x: xDirection * gridSize, y: yDirection * gridSize }
}

function sharedTerminalInset(
  candidates: RouteEndpointInset[],
): RouteEndpointInset | null {
...
/**
 * Keeps a manually dragged segment one grid step inside an element anchor's
 * required approach direction. Without this lead point, the orthogonal router
 * has to backtrack after the new waypoint and renders a short dangling stub.
 */
export function insetDraggedRouteSegmentFromElementAnchors(
  networks: ConnectionNetwork[],
  ...
  const startInsets: RouteEndpointInset[] = []
  const endInsets: RouteEndpointInset[] = []
  ...
  const collectInset = (
    ...
    const network = networksById.get(route.networkId)
    if (!network || route.points.length < 2) return
    const nodesById = new Map(network.nodes.map((node) => [node.id, node]))
    if (
      pointsEqual(segmentPoint, route.points[0]) &&
      nodesById.get(route.sourceNodeId)?.kind === 'element-anchor'
    ) {
      const inset = terminalInsetTowardAdjacentPoint(
        route.points[0],
        route.points[1],
        gridSize,
      )
      if (inset) candidates.push(inset)
    }
    const lastIndex = route.points.length - 1
    if (
      pointsEqual(segmentPoint, route.points[lastIndex]) &&
      nodesById.get(route.targetNodeId)?.kind === 'element-anchor'
    ) {
      const inset = terminalInsetTowardAdjacentPoint(
  ...
  routedEdges.forEach((route) => {
    if (!participantEdgeIds.has(route.edgeId)) return
    collectInset(route, segment.start, startInsets)
    collectInset(route, segment.end, endInsets)
  })

  const startInset = sharedTerminalInset(startInsets)
  const endInset = sharedTerminalInset(endInsets)
  return {
    start: startInset
      ? { x: nextStart.x + startInset.x, y: nextStart.y + startInset.y }
      : nextStart,
    end: endInset
      ? { x: nextEnd.x + endInset.x, y: nextEnd.y + endInset.y }
      : nextEnd,
  }
}
