...
describe('AIDC editor workspace', () => {
  ...
  it('switches to a read-only monitor workspace and saves On/Off and pump states', async () => {
    ...
    expect(screen.getByTestId('diagram-monitor-canvas')).toHaveAttribute('data-animation-mode', 'dashes')
    await user.click(screen.getAllByRole('button', { name: '园区总图' })[1])
    expect(screen.getByTestId('diagram-monitor-canvas')).toHaveAttribute('data-animation-mode', 'dots')
    await user.click(screen.getByRole('button', { name: '光波' }))
    await user.click(screen.getAllByRole('button', { name: '园区总图' })[0])
    expect(screen.getByTestId('diagram-monitor-canvas')).toHaveAttribute('data-animation-mode', 'dashes')
    await user.click(screen.getAllByRole('button', { name: '园区总图' })[1])
