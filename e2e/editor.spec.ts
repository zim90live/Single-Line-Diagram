...
test('uses an immersive canvas with floating AIDC workspace panels', async ({ page }) => {
  await page.goto('/')

  const appShell = page.getByTestId('app-shell')
  const topbar = page.locator('.workspace-header')
  ...
  const canvasTitlebar = canvasFrame.locator('.canvas-titlebar')
  const canvasActionbar = canvasFrame.locator('.canvas-actionbar')
  const selectedTreeRow = page.locator('.tree-row[data-selected="true"]').first()
  await expect(page.locator('.context-toolbar')).toHaveCount(0)
  await expect(page.locator('.ruler, .ruler-corner')).toHaveCount(0)
  await expect(canvasTitlebar).toHaveCount(1)
  ...
  await expect(page.getByRole('searchbox', { name: '搜索图纸层级' })).toBeVisible()
  ...
  await expect(topbar).toHaveCSS('background-color', 'rgba(28, 28, 28, 0.3)')
  ...
  await expect(canvasTitlebar).toHaveCSS('min-height', '44px')
  await expect(canvasTitlebar).toHaveCSS('border-radius', '12px')
  await expect(canvasTitlebar).toHaveCSS('background-color', 'rgba(0, 0, 0, 0)')
  await expect(leftSidebar).toHaveCSS('background-color', 'rgba(28, 28, 28, 0.9)')
  await expect(leftSidebar).toHaveCSS('border-top-width', '0px')
  await expect(propertiesPanel).toHaveCSS('background-color', 'rgba(28, 28, 28, 0.9)')
  await expect(propertiesPanel).toHaveCSS('padding-top', '24px')
  await expect(propertiesPanel).toHaveCSS('border-top-width', '0px')
  await expect(selectedTreeRow).toHaveCSS('height', '28px')
  await expect(selectedTreeRow).toHaveCSS('background-color', 'rgba(255, 255, 255, 0.08)')
  await expect(selectedTreeRow.locator('.tree-row__icon')).toHaveCSS('width', '18px')
  expect(await selectedTreeRow.locator('.tree-row__icon').evaluate((element) => element.tagName))
    .toBe('IMG')
  await expect(canvasActionbar).toHaveCSS('border-radius', '6px')
  await expect(canvasActionbar).toHaveCSS('box-shadow', 'none')
  const editPanelShadows = await Promise.all([
    leftSidebar.evaluate((element) => getComputedStyle(element).boxShadow),
    propertiesPanel.evaluate((element) => getComputedStyle(element).boxShadow),
  ])
  expect(editPanelShadows).toEqual(['none', 'none'])
  ...
  const treeBox = await leftSidebar.boundingBox()
  const inspectorBox = await propertiesPanel.boundingBox()
  const breadcrumbBox = await canvasTitlebar.boundingBox()
  if (!appBox || !canvasBox || !treeBox || !inspectorBox || !breadcrumbBox) {
    throw new Error('无法读取沉浸式工作区布局')
  }
  expect(canvasBox.x).toBeCloseTo(appBox.x, 0)
  expect(canvasBox.y).toBeCloseTo(appBox.y, 0)
  expect(canvasBox.width).toBeCloseTo(appBox.width, 0)
  expect(canvasBox.height).toBeCloseTo(appBox.height, 0)
  expect(treeBox.x - appBox.x).toBeCloseTo(4, 0)
  expect(treeBox.y - appBox.y).toBeCloseTo(68, 0)
  expect(treeBox.width).toBeCloseTo(280, 0)
  expect(appBox.x + appBox.width - treeBox.x - treeBox.width).toBeGreaterThan(0)
  expect(inspectorBox.y - appBox.y).toBeCloseTo(68, 0)
  expect(inspectorBox.width).toBeCloseTo(360, 0)
  expect(appBox.x + appBox.width - inspectorBox.x - inspectorBox.width).toBeCloseTo(4, 0)
  expect(breadcrumbBox.x - appBox.x).toBeCloseTo(288, 0)
  expect(breadcrumbBox.y - appBox.y).toBeCloseTo(68, 0)

  await page.getByRole('tab', { name: /监控模式/ }).click()
  await expect(page.locator('.workspace-main')).toHaveAttribute('data-mode', 'monitor')
  await expect(leftSidebar).toHaveCSS('background-color', 'rgba(28, 28, 28, 0.9)')
  await expect(leftSidebar).toHaveCSS('box-shadow', 'none')
  await expect(page.getByRole('searchbox', { name: '搜索监控图纸' })).toBeVisible()
  const monitorTreeBox = await leftSidebar.boundingBox()
  const monitorInspectorBox = await propertiesPanel.boundingBox()
  const monitorCanvasBox = await canvasFrame.boundingBox()
  if (!monitorTreeBox || !monitorInspectorBox || !monitorCanvasBox) {
    throw new Error('无法读取监控模式悬浮面板布局')
  }
  expect(monitorTreeBox.x - appBox.x).toBeCloseTo(4, 0)
  expect(monitorTreeBox.y - appBox.y).toBeCloseTo(68, 0)
  expect(monitorTreeBox.width).toBeCloseTo(280, 0)
  expect(monitorInspectorBox.width).toBeCloseTo(360, 0)
  expect(appBox.x + appBox.width - monitorInspectorBox.x - monitorInspectorBox.width)
    .toBeCloseTo(4, 0)
  expect(monitorCanvasBox.width).toBeCloseTo(appBox.width, 0)
  expect(monitorCanvasBox.height).toBeCloseTo(appBox.height, 0)
})
...
test('collapses and expands the left menu from its graphical edge control', async ({ page }) => {
  ...
  expect(expandedCanvasBox.width).toBeCloseTo(initialCanvasBox.width, 0)
  expect(expandedCanvasBox.x).toBeCloseTo(initialCanvasBox.x, 0)
  const expandButtonBox = await page.getByRole('button', { name: '展开左侧菜单' }).boundingBox()
  if (!expandButtonBox) throw new Error('无法读取展开按钮位置')
  expect(expandButtonBox.x).toBeCloseTo(4, 0)
